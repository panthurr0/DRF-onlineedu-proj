...
class CourseTestCase(APITestCase):
    def setUp(self):
        ...
        self.lesson = Lesson.objects.create(
            lesson_title="Урок о сём", course=self.course, owner=self.user
        )
    ...
    def test_course_create(self):
        ...
        data = {"course_title": "Курс такойто"}
    ...
    def test_course_update(self):
        ...
        self.assertEqual(response_data.get("course_title"), "Курс такойто")
...
class LessonTestCase(APITestCase):
    ...
    def test_lesson_create(self):
        ...
        data = {"lesson_title": "Урок не о чём"}
        response = self.client.post(url, data)
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
    ...
    def test_lesson_update(self):
        ...
        data = {"lesson_title": "Урок такойто"}
        response = self.client.patch(url, data)
        response_data = response.json()
        self.assertEqual(response.status_code, status.HTTP_200_OK)

        self.assertEqual(response_data.get("lesson_title"), "Урок такойто")
